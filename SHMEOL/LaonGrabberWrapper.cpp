...
void CLaonGrabberWrapper::CopyInspImage(int nType, BYTE* GrabImage , bool sumImageSave)
{
	...
	else if (nType == OC_RAW_IMAGE)
	{
		str.Format("OcRaw");
		memcpy(g_clLaonGrabberWrapper[m_nUnit].vDefectMidBuffer_3000k, GrabImage, g_clLaonGrabberWrapper[m_nUnit].m_pBoard->GetFrameRawSize());
		RawImageSave(str, g_clLaonGrabberWrapper[m_nUnit].vDefectMidBuffer_3000k);
	}
	else if (nType == OC_YUV_IMAGE)
	{
		str.Format("OcYuv");
		memcpy(g_clLaonGrabberWrapper[m_nUnit].vDefectMidBuffer_3000k, GrabImage, g_clLaonGrabberWrapper[m_nUnit].m_pBoard->GetFrameRawSize());
		RawImageSave(str, g_clLaonGrabberWrapper[m_nUnit].vDefectMidBuffer_3000k);
...
void CLaonGrabberWrapper::RawImageSave(CString nType , BYTE* LoadImg, bool mJpgSaveUse)
{
	...
	_stprintf_s(szFilePath, SIZE_OF_1K, _T("%s\\%s_%s_%02d%02d%02d_100.raw"), szPath, cnamePath, nType, stSysTime.wHour, stSysTime.wMinute, stSysTime.wSecond);
#else
	_stprintf_s(szFilePath, SIZE_OF_1K, _T("%s\\%s_%s_%02d%02d%02d_150.raw"), szPath, cnamePath, nType, stSysTime.wHour, stSysTime.wMinute, stSysTime.wSecond);
#endif
