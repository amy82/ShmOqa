...
#define POSITION_END(x, bsize, size0, ch)		((x == 0 ? size0 : (x * bsize + size0)) * ch - 1)
	...
	ACMISSOFTISP_API void GetMeanFromBayerDouble(const BYTE* pRaw, int width, int height, int l, int t, int w, int h, double* mean);
	ACMISSOFTISP_API void GetMeanFromBayerDouble(const BYTE* pRaw, int width, int height, RECT rtROI, double* mean);
	ACMISSOFTISP_API void GetMeanFromBayerDouble(const int* pRaw, int width, int height, int l, int t, int w, int h, double* mean);
	ACMISSOFTISP_API void GetMeanFromBayerDouble(const int* pRaw, int width, int height, RECT rtROI, double* mean);
	ACMISSOFTISP_API void GetMeanFromBMPdouble(const int* pData, int width, int height, int leftROI, int topROI, int rightROI, int bottomROI, double* mean, int color);
	ACMISSOFTISP_API void GetMeanFromBMPdouble(const int* pData, int width, int height, RECT rtROI, double* mean, int color);
	ACMISSOFTISP_API void GetMeanFromYdouble(const BYTE* pData, int width, int height, int leftROI, int topROI, int rightROI, int bottomROI, double* mean);
	ACMISSOFTISP_API void GetMeanFromYdouble(const BYTE* pData, int width, int height, RECT rtROI, double* mean);
	ACMISSOFTISP_API void GetMeanFromYdouble(const int* pData, int width, int height, int leftROI, int topROI, int rightROI, int bottomROI, double* mean);
	ACMISSOFTISP_API void GetMeanFromYdouble(const int* pData, int width, int height, RECT rtROI, double* mean);
	...
	ACMISSOFTISP_API void SeparateCFA(double* pBuffer, int nImageWidth, int nImageHeight, double* pChannelLT, double* pChannelRT, double* pChannelLB, double* pChannelRB);
	...
	ACMISSOFTISP_API bool DetectPatchVertexsOpenCV(const BYTE *pBuffer, int nImageWidth, int nImageHeight, void *params, std::vector<TPatchInfo> vPatchInfo, THarrisCornerPoint* outputPos, std::vector<TPatchSize>& vPatchSize);
	ACMISSOFTISP_API bool DetectPatchVertexsOpenCV(const BYTE *pBuffer, int nImageWidth, int nImageHeight, double dThreshold, TPatchInfo tPatchInfo, int nPatchIndex, THarrisCornerPoint* outputPos, TPatchSize& tPatchSize);
