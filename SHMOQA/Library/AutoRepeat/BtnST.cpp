	...
	switch (m_byAlign)
	{
		case ST_ALIGN_HORIZ:
			if (bHasTitle == FALSE)
			{
				...
				rpImage->left += (LONG)((rpImage->Width() - (long)dwWidth)*0.5);
			}
			else
			{
			...
			rpImage->top += (LONG)((rpImage->Height() - (long)dwHeight)*0.5);
			break;
...
void CButtonST::DrawTheText(CDC* pDC, LPCTSTR lpszText, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	...
	rpCaption->OffsetRect((centerRect.Width() - rpCaption->Width())/2, (int)((centerRect.Height() - rpCaption->Height())*0.5));
	/* RFU
...
DWORD CButtonST::OffsetColor(BYTE byColorIndex, short shOffset, BOOL bRepaint)
{
	...
	if (shOffset > 0)
	{
		...
		shOffset = __min(__min(shOffsetR, shOffsetG), shOffsetB);
	} // if
	else
	{
		...
		shOffset = __max(__max(shOffsetR, shOffsetG), shOffsetB);
	} // else 
