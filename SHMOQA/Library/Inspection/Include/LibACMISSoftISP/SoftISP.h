...
namespace ACMISSoftISP
{
	...
	ACMISSOFTISP_API bool xSaturation(const int* pBuffer, const BYTE* pFrameY, int nImageWidth, int nImageHeight, TDATASPEC tDataSpec, RECT rtROI, double dBinaryThreshold, double& dROIBrightMin, double& dROIBrightMax, double& dROIBrightAvg, POINT& ptMin, POINT& ptMax, int& nROIBrightMaxBin, int& nROIBrightMaxBinPixelCnt);
