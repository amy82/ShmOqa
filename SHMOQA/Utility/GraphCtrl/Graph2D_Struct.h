...
struct strGraph2D_Axis_Info	//! 축, Grid, 커서 관련 설정 구조체
{
	...
	strGraph2D_Axis_Info& operator=(const strGraph2D_Axis_Info &input)
	{
		...
		//if(bDirection_Plus == false) AfxMessageBox("false");
...
struct strValueLineXInfo		//! X축 특정값 설정 구조체
{
	...
	void InitInfo()
	{
		...
		dbValue = 0.;
		clrValue = RGB(255, 255, 255);//RGB(0, 0, 0);	안쪽 박스 배경색
		iLineWidth = 1;
		szText = _T("");
	}
...
struct strGraph2DTitleInfo					//! 제목 설정 구조체
{
	...
	void InitInfo()
	{
		...
		dbLeftMargin = 0.;
		dbRightMargin = 0.;
		clrTitle = RGB(0, 0, 0);//RGB(255, 255, 255);
		szTitle = _T("");
...
struct strGraph2DResourceInfo				//! 색상, X축 설정 구조체
{
	...
	void InitInfo()
	{
		...
		/*clrEntireBackGround = RGB(18,26,33);
		clrGraphBackGround = RGB(100, 100, 102);
		clrAxis = RGB(231, 231, 231);
		clrGrid = RGB(255, 255, 255);		
		clrGridText = RGB(255, 255, 255);*/
