#include "StdAfx.h"
#include "Handler_RS232C.h"

CList_RS232C::CList_RS232C(void)
{
...
bool CList_RS232C::GetList_SerialPort()
{
	HKEY hResKey = NULL;
...
CHelper_Setup_RS232C::CHelper_Setup_RS232C(void)
{
}
...
bool CHelper_Setup_RS232C::GetValue_BaudRate(int iIndex_BaudRate, DWORD &dwRetBaudRate, CString *psItemName)
{
	bool bRet = true;
...
bool CHelper_Setup_RS232C::GetValue_DataBit(int iIndex_DataBit, BYTE  &byRetDataBit, CString *psItemName)
{
	bool bRet = true;
...
bool CHelper_Setup_RS232C::GetValue_StopBit(int iIndex_StopBit, BYTE  &byRetStopBit, CString *psItemName)
{
	bool bRet = true;
...
bool CHelper_Setup_RS232C::GetValue_ParityBit(int iIndex_ParityBit, BYTE  &byRetParityBit, CString *psItemName)
{
	bool bRet = true;
...
int CHelper_Setup_RS232C::GetIndex_BaudRate(DWORD dwBaudRate)
{
	int iIndex_Ret = -1;
...
int CHelper_Setup_RS232C::GetIndex_DataBit(BYTE byDataBit)
{
	int iIndex_Ret = -1;
...
int CHelper_Setup_RS232C::GetIndex_StopBit(BYTE byStopBit)
{
	int iIndex_Ret = -1;
...
int CHelper_Setup_RS232C::GetIndex_ParityBit(BYTE byParityBit)
{
	int iIndex_Ret = -1;
...
//! [반환값] Baud Rate 정수형 값
int CHelper_Setup_RS232C::GetBaudRateValue_Integer_FromConstant(DWORD dwBaudRate)
{
	int iRet = 38400;
...
int CHelper_Setup_RS232C::Convert_Integer_To_ArrayByte(int iInput, BYTE* pbyOutputBuf, int iOutputBufSize)
{
...
CHandler_RS232C::CHandler_RS232C(void)
{
	//! 초기화
	m_hComm = NULL;
...
DWORD CHandler_RS232C::GetMask_CPU_Affinity(int *piBuf_CPU_Use, int iBufSize_CPU_Use)
{
...
int	CHandler_RS232C::Set_CPU_Affinity(int *piBuf_CPU_Use, int iBufSize_CPU_Use)
{
	//! 아직, Thread가 생성되지 않았음
	...
	if ( iCPU_Size > iBufSize_RS232C_CPU_Core )
	{
		iSize_Possible_CPU_Core = iBufSize_RS232C_CPU_Core;
	}
	else
	{
...
//! [기능] Thread 작동을 시작한다.
int CHandler_RS232C::StartThread(bool bFlag_Use_CPU_Affinity, int *piBuf_CPU_Use, int iBufSize_CPU_Use)
{
	if ( m_pThread_RecvComm != NULL )
	{
		return -1;
	...
	//! Thread 생성
	m_pThread_RecvComm = AfxBeginThread(Thread_Receive_Comm, this, THREAD_PRIORITY_NORMAL, 0, 0);
	...
	int iRet = Set_CPU_Affinity(m_strCommSetupInfo.aiCPU, iBufSize_RS232C_CPU_Core);
	if ( iRet < 0 )
	{
		return (-10 + iRet);
	}
...
int CHandler_RS232C::Stop_Thread()
{
	if ( m_pThread_RecvComm == NULL )
	{
		return -1;
...
int CHandler_RS232C::Stop_Thread_Waiting(int iTimeOut)
{
	if ( m_pThread_RecvComm == NULL )
	{
		return -1;
	...
	if ( bRet == true )
	{
		...
		TRACE(_T("CHandler_RS232C::Stop_Thread_Waiting [%s] Stop OK \n"), m_strCommSetupInfo.sPortName);
	}
	else
	{
		TRACE(_T("CHandler_RS232C::Stop_Thread_Waiting [%s] Stop Error \n"), m_strCommSetupInfo.sPortName);
...
void CHandler_RS232C::SetReceiveProcPtr(void* pInput)
{
	m_csReceiveProcPtr.Lock();
...
//! [기능] 수신 Thread 본체 함수
UINT Thread_Receive_Comm(LPVOID pParam)
{
	CHandler_RS232C *pSerial = (CHandler_RS232C*)pParam;
	...
	while(1)
	{
		...
		if ( (dwEvent & EV_RXCHAR) == EV_RXCHAR )
		{
			...
			do
			{
				...
				/*if (dwReceive > 0 && g_bReady_UV==false)
				{
					CString tmp;
				...
				//! byRecvBuff의 데이터를 전달하여, CHandler_RS232C 클래스에서 상속받은 클래스에서 처리하도록 한다. 
...
bool CHandler_RS232C::IsConnected()
{
	return m_strCommStateInfo.bFlag_Connected;
}

CString CHandler_RS232C::GetCurrent_PortName()
{
	return m_strCommSetupInfo.sPortName;
}

DWORD CHandler_RS232C::GetCurrent_BaudRate()
{
	return m_strCommSetupInfo.dwBaudRate;
}

BYTE CHandler_RS232C::GetCurrent_DataBit()
{
	return m_strCommSetupInfo.byDataBit;
}

BYTE CHandler_RS232C::GetCurrent_StopBit()
{
	return m_strCommSetupInfo.byStopBit;
}

BYTE CHandler_RS232C::GetCurrent_ParityBit()
{
...
int CHandler_RS232C::OpenPort(strSetupInfo_RS232C *pstrSetupInfo)
{
	...
	//! 수신 Thread 생성
	int iRet = StartThread(m_strCommSetupInfo.bFlag_Use_CPU_Affinity, m_strCommSetupInfo.aiCPU, iBufSize_RS232C_CPU_Core);
	if ( iRet < 0 )
	{
		return (-100 + iRet);
	}
...
DWORD CHandler_RS232C::WriteComm(CString szSend, bool bSTX_ETX)
{
...
//! [반환값] 실제로 송신한 Byte 개수를 반환한다. 
//! [주의]   송신할 문자열의 크기에 제한이 있다. 현재는 영문자, 숫자 기준으로 8190자까지 송신할 수 있다. 
DWORD CHandler_RS232C::WriteComm(BYTE *pbyBuff, DWORD nSizeToSend, bool bSTX_ETX)
{
...
DWORD CHandler_RS232C::ReceiveComm(BYTE *pbyBuff, DWORD nSizeToReceive)
{
...
//! 통신 마침 함수
void CHandler_RS232C::ClosePort()
{
...
void CHandler_RS232C::ClosePort(int iTimeOut)
{
